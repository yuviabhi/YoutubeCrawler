...
class YoutubeImportHandler extends ImportHandler
{
    ...
    public function process_src_document($src_document)
    {
        ...
        if ($youtube_id == null) {
            print_r("Youtube ID issue. Ignored : " . $src_document->etag . PHP_EOL);
        } else {
    ...
    public function process_all_documents($data_src)
    {
        ...
        //$structure = new NDLIStructure($this->src_id);
        ...
        foreach ($playlists as $playlist) {
            ...
            //if (isset($data->items[0]->snippet->playlistId)) {
                //$this->col_id = $data->items[0]->snippet->playlistId;
                //$this->col_name = ucwords($playlist);
                //$structure->add_collection($this->col_id, $this->col_name);
           //}
